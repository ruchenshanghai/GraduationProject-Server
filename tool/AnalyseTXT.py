...
def get_topic_list_by_tags(tag_list):
    session = DBsession()
    topic_list = []
...
stoplist = {}.fromkeys([ line.strip() for line in open("../resource/stop_word.txt") ])
word_list = jieba.cut(test_str, cut_all=True)
word_list = [word.encode('utf-8') for word in list(word_list)]

word_list = [word for word in list(word_list) if word not in stoplist]
